...
def main(input_folder: Path = Path("nbs"), output_folder: Path = Path("src")):
    """This is help text maybe?"""
    ...
    for file in files:
        ...
        try:
            ...
            codes = {k: v.code for k, v in app.graph.cells.items() if v.language=="python" and "## Export" in v.code}
            code_export = ""
            for i in order:
                if i in codes:
